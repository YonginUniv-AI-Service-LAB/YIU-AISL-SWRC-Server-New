...
public class AdminApprovalController {
    ...
    // ✅ 승인 대기 관리자 목록 조회
    @GetMapping("/pending")
    public ResponseEntity<List<Admin>> getPendingAdmins(@AuthenticationPrincipal User user) {
    ...
    // ✅ 승인된 관리자 목록 조회
    @GetMapping("/approved")
    public ResponseEntity<List<Admin>> getApprovedAdmins(@AuthenticationPrincipal User user) {
        validateSuperAdmin(user);
    ...
    private void validateSuperAdmin(User user) {
        if (user == null || !SUPER_ADMIN_EMAIL.equals(user.getEmail())) {
